...
Initialization:
	...
	li $t0, 0x19
	sw $t0, print_cursor_x
	#print_char_address = 0x1a0
	li $t0, 0x1a0
	sw $t0, print_char_address
...
Display_words:
	...
	# $t0 = ascii
	lw $t8, print_char_address
	lw $t0, 0x0($t8)
	addi $t8, $t8, 4
	sw $t8, print_char_address
...
Print_char:
	...
	lw $a0, print_cursor_x
	lw $a1, print_cursor_y
	jal xy2vram_address
...
next_print_cursor_y:
	lw $t0, print_cursor_y
	bne $t0, $zero, npcy_minus_one
	li $t0, 59
	sw $t0, print_cursor_y
	jr $ra
