...
is_Caps:	#When release Caps_lock_key, change the state.
	...
	bne $a0, $t0, is_Fx	#not Caps
	bne $a1, $zero, is_Other#is Caps, but not released, ignore it.
	ori $t1, $zero, 0x14c	#load current caps state
	lw  $t2, 0($t1)
...
is_Fx:
	ori $t0, $zero, 0xd
	slt $t1, $a0, $t0	# $t1 = scan_code < 0xd ?
	beq $t1, $zero, is_Not_used# $t1 == 0, >= 0xd
	#beq $t1, $zero, is_Shift# $t1 == 0, >= 0xd
...
is_Not_used:
	ori $t0, $zero, 0x66
	slt $t1, $t0, $a0	# $t1 = 0x66 < scan_code ?
	beq $t1, $zero, is_Shift
...
Caps_convert:
	...
	ori $t0, $zero, 0x14c	# $s0 = isCapsOn?
	lw $s2, 0($t0)
	ori $t0, $zero, 0x144	# $s1 = isShiftPressed?
	lw $s3, 0($t0)
